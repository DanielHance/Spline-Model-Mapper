...
void Shader::render()
{
    vec3 a;
    vec3 b;
    render_gen(false, a, b);
}

void Shader::render_gen(bool _line, const vec3& _a, const vec3& _b)
{
    id();
    int count = -1;
    ...
    {
        GLint posLoc = glGetAttribLocation(id(), "a_Position");
        if (posLoc == -1) throw Exception("a_Position not found in shader!");
        glBindVertexArray(0);
        float dat[] = {
            _a.x, _a.y, _a.z,
            _b.x, _b.y, _b.z
        };
        glVertexAttribPointer(posLoc, 3, GL_FLOAT, GL_FALSE, 0, dat);
...
void Shader::render_line(const vec3& _a, const vec3& _b)
{
    render_gen(true, _a, _b);
