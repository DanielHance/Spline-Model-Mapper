...
std::vector<Face> Model::getFaces()
{
    ...
    for (sys::linked_list<Part>::iterator pit = m_parts.begin(); pit != m_parts.end(); ++pit)
    {
        for (sys::linked_list<MaterialGroup>::iterator mit = pit->m_mgs.begin(); mit != pit->m_mgs.end(); ++mit)
        {
            for (int fit = 0; fit < mit->mesh.m_faces.size(); fit++)
            {
                rtn.push_back(mit->mesh.m_faces[fit]);
            }
        }
    }
    std::cout << rtn.size() << std::endl;
...
glm::vec3 Model::getMax()
{
    ...
    for (sys::linked_list<Part>::iterator pit = m_parts.begin(); pit != m_parts.end(); ++pit)
    {
        for (sys::linked_list<MaterialGroup>::iterator mit = pit->m_mgs.begin(); mit != pit->m_mgs.end(); ++mit)
        {
            if (max.x < mit->mesh.getMax().x) max.x = mit->mesh.getMax().x;
            if (max.y < mit->mesh.getMax().y) max.y = mit->mesh.getMax().y;
            if (max.z < mit->mesh.getMax().z) max.z = mit->mesh.getMax().z;
        }
    }
...
glm::vec3 Model::getMin()
{
    ...
    for (sys::linked_list<Part>::iterator pit = m_parts.begin(); pit != m_parts.end(); ++pit)
    {
        for (sys::linked_list<MaterialGroup>::iterator mit = pit->m_mgs.begin(); mit != pit->m_mgs.end(); ++mit)
        {
            if (min.x < mit->mesh.getMin().x) min.x = mit->mesh.getMin().x;
            if (min.y < mit->mesh.getMin().y) min.y = mit->mesh.getMin().y;
            if (min.z < mit->mesh.getMin().z) min.z = mit->mesh.getMin().z;
        }
    }
