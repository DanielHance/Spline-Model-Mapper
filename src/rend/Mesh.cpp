...
{
    _face.min = vec3(99999999999999999);
    _face.max = vec3(-9999999999999999);

    //v1
    if (_face.max.x < _face.a.position.x) _face.max.x = _face.a.position.x;
    if (_face.max.y < _face.a.position.y) _face.max.y = _face.a.position.y;
    if (_face.max.z < _face.a.position.z) _face.max.z = _face.a.position.z;
    if (_face.min.x > _face.a.position.x) _face.min.x = _face.a.position.x;
    if (_face.min.y > _face.a.position.y) _face.min.y = _face.a.position.y;
    if (_face.min.z > _face.a.position.z) _face.min.z = _face.a.position.z;
    //v2
    if (_face.max.x < _face.b.position.x) _face.max.x = _face.b.position.x;
    if (_face.max.y < _face.b.position.y) _face.max.y = _face.b.position.y;
    if (_face.max.z < _face.b.position.z) _face.max.z = _face.b.position.z;
    if (_face.min.x > _face.b.position.x) _face.min.x = _face.b.position.x;
    if (_face.min.y > _face.b.position.y) _face.min.y = _face.b.position.y;
    if (_face.min.z > _face.b.position.z) _face.min.z = _face.b.position.z;
    //v3
    if (_face.max.x < _face.c.position.x) _face.max.x = _face.c.position.x;
    if (_face.max.y < _face.c.position.y) _face.max.y = _face.c.position.y;
    if (_face.max.z < _face.c.position.z) _face.max.z = _face.c.position.z;
    if (_face.min.x > _face.c.position.x) _face.min.x = _face.c.position.x;
    if (_face.min.y > _face.c.position.y) _face.min.y = _face.c.position.y;
    if (_face.min.z > _face.c.position.z) _face.min.z = _face.c.position.z;

    //Max/Min
    if (m_max.x < _face.max.x) m_max.x = _face.max.x;
    if (m_max.y < _face.max.y) m_max.y = _face.max.y;
    if (m_max.z < _face.max.z) m_max.z = _face.max.z;
    if (m_min.x > _face.min.x) m_min.x = _face.min.x;
    if (m_min.y > _face.min.y) m_min.y = _face.min.y;
    if (m_min.z > _face.min.z) m_min.z = _face.min.z;

    /*std::cout << _face.min.x << ", " << _face.min.y << ", " << _face.min.z << "  min" << std::endl;
    std::cout << _face.max.x << ", " << _face.max.y << ", " << _face.max.z << "  max" << std::endl;*/
