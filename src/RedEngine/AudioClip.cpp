...
namespace red
{
	...
	void AudioClip::onLoad()
	{
		...
		alBufferData(m_buffer, format, &bufferData.at(0), static_cast<ALsizei>(bufferData.size()), freq);

	}

	void AudioClip::load_ogg(const std::string& _path, std::vector<unsigned char>& _buffer,
